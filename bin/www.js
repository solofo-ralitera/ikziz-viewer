...
Http.createServer(function(request, response) {
    ...
    aUrl.shift(); // remove frst empty element
    try {
        let method = eval(aUrl.shift());
        aUrl = aUrl.map(function(url){
            return decodeURIComponent(url);
        });
        if(method) {
            return method.apply(method, [].concat(response, parsedUrl, aUrl));
        }else {
            response.end('');
        }
    }catch(e) {
...
function sendResponse(response, status, res) {
 ...
 * @param response
 */
function allBooks(response, parsedUrl) {
    readDir(
        // Read root dir
        ROOT
    ).then((ret) => {
        ...
        ImageLists.forEach((dirs, idx) => {
            dirs.forEach((dir) => {
                try {
                    if(/\.db$/.test(dir)) return false;

                    let firstFile = null;
                    let files = Fs.readdirSync(dir);
                    if(Array.isArray(files)) {
                        firstFile = files.shift();
                    }
                    if(firstFile !== null) {
                        aDir = dir.split("/");
                        ...
                        ret.push({
                            ...
                            thumb: dir.replace(ROOT, "") + "/" + firstFile,
        ...
        // Pagination
        let offset = 0;
        let limit = ret.length;
        if(typeof parsedUrl.query.offset != "undefined") {
            offset = parseInt(parsedUrl.query.offset);
        }
        if(typeof parsedUrl.query.limit != "undefined") {
 ...
 * @param parsedUrl
 */
function image(response, parsedUrl) {
    let imagePath = [];
    imagePath.push(ROOT);
    for(let i = 2; i < arguments.length; i++) {
        if(typeof arguments[i] == "string") {
            imagePath.push(arguments[i]);
        }
    }
    let image = imagePath.join("/");
 ...
 * @param parsedUrl
 */
function book(response, parsedUrl, path) {
    ...
    readDir(
        ...
        aPath.shift();
        let ret = {
            author : aPath[0].authorForHuman(),
            title : aPath[1].titleForHuman(),
            width : 0,
            height : 0,
            files : []
        };
        ...
        files.forEach((file) => {
            if(/jpg|jpeg|png|gif$/.test(file)) {
                imageDim = sizeOf(file);
                ret.width = Math.max(ret.width, imageDim.width);
                ret.height = Math.max(ret.width, imageDim.height);
                ret.files.push(file.replace(ROOT, "").replace(/\/{1,}/, "/"));
            }
        });
        ret.files.sort(function(a,b) {
            let sa = parseInt(a.split("/").pop().replace(/[^0-9]/g, ""));
